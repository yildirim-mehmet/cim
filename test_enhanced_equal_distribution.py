...
def test_normal_scenarios():
    """Test equal distribution across normal scenarios"""
...
def test_extreme_imbalance():
    """Test equal distribution under extreme existing duty imbalance"""
    print("\n=== TESTING EXTREME IMBALANCE SCENARIOS ===")
    ...
    if person_counts:
        ...
        if difference > 1:
            print(f'❌ ENHANCED ALGORITHM STILL FAILS: Difference = {difference}')
            return False
        else:
            print(f'✅ Enhanced algorithm handles extreme imbalance (difference = {difference})')
            return True
    
    return False

def test_constraint_preservation():
    """Test that enhanced scoring preserves existing constraints"""
    print("\n=== TESTING CONSTRAINT PRESERVATION ===")
...
if __name__ == "__main__":
    print("🧪 TESTING ENHANCED EQUAL DISTRIBUTION ALGORITHM")
    
    normal_test = test_normal_scenarios()
    extreme_test = test_extreme_imbalance()
    ...
    if normal_test and extreme_test and constraint_test:
        print("🎉 ALL TESTS PASSED - Enhanced equal distribution working!")
    else:
        print("💥 SOME TESTS FAILED - Algorithm needs further improvement")
