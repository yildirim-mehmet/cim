...
class DutyScheduler:
    """
    Nöbet Programı Zamanlayıcısı - USER PROVEN + NEW REQUIREMENTS
    ...
    - Zorunlu Pazar-Pazartesi eşleştirmesi (farklı haftalarda) - NEW
    - İlk 7 gün analizi ve sonraki hafta eşleştirmeleri - NEW
    - Aynı gün türü tekrar atama yüzdelik kısıtlamaları - NEW
    - Kritik gün dağıtım kontrolü - NEW
    ...
    def calculate_pairing_bonus(self, person_id, target_date, existing_schedule, year, month):
        """
        Zorunlu eşleştirme bonusu hesaplama - USER PROVEN + NEW REQUIREMENTS
        ...
        if target_weekday == 3:  # Perşembe atanıyor
            has_saturday = any(scheduled_date.weekday() == 5 for scheduled_date, scheduled_person, _ in existing_schedule 
                             if scheduled_person == person_id and scheduled_date.year == year and scheduled_date.month == month)
            if not has_saturday:
                bonus += 800  # ULTRA GÜÇLÜ: Perşembe atanıyor, Cumartesi yok - eşleştirme fırsatı
        elif target_weekday == 5:  # Cumartesi atanıyor
            has_thursday = any(scheduled_date.weekday() == 3 for scheduled_date, scheduled_person, _ in existing_schedule 
                             if scheduled_person == person_id and scheduled_date.year == year and scheduled_date.month == month)
            if has_thursday:
                bonus += 2500  # ULTRA MEGA BONUS: Perşembe var, Cumartesi ile eşleştirme tamamlanıyor!
            else:
                bonus += 800  # GÜÇLÜ: Cumartesi atanıyor, Perşembe yok - eşleştirme fırsatı
        
        if target_weekday == 4:  # Cuma atanıyor
            has_sunday = any(scheduled_date.weekday() == 6 for scheduled_date, scheduled_person, _ in existing_schedule 
                           if scheduled_person == person_id and scheduled_date.year == year and scheduled_date.month == month)
            if not has_sunday:
                bonus += 800  # ULTRA GÜÇLÜ: Cuma atanıyor, Pazar yok - eşleştirme fırsatı
        elif target_weekday == 6:  # Pazar atanıyor - ÇOKLU EŞLEŞTİRME LOGİĞİ
            ...
            if has_friday:
                bonus += 2500  # ULTRA MEGA BONUS: Cuma var, Pazar ile eşleştirme tamamlanıyor!
            if not has_monday:
                bonus += 1200  # ULTRA GÜÇLÜ: Pazar atanıyor, Pazartesi yok - eşleştirme fırsatı
            if not has_friday and not has_monday:
                bonus += 800  # GÜÇLÜ: Pazar atanıyor, hiç eşleştirme yok - temel bonus
        elif target_weekday == 0:  # Pazartesi atanıyor
            has_sunday = any(scheduled_date.weekday() == 6 for scheduled_date, scheduled_person, _ in existing_schedule 
                           if scheduled_person == person_id and scheduled_date.year == year and scheduled_date.month == month)
            if has_sunday:
                bonus += 3000  # ULTIMATE MEGA BONUS: Pazar var, Pazartesi ile eşleştirme tamamlanıyor!
            else:
                bonus += 1200  # ULTRA GÜÇLÜ: Pazartesi atanıyor, Pazar yok - eşleştirme fırsatı
    ...
    def needs_sunday_monday_pairing(self, person_id, target_date, existing_schedule, year, month):
        ...
        if target_weekday not in [0, 6]:  # Pazartesi=0, Pazar=6
            return False
        ...
        for scheduled_date, scheduled_person, _ in existing_schedule:
            if scheduled_person == person_id and scheduled_date.year == year and scheduled_date.month == month:
                ...
                if scheduled_weekday == 6:  # Pazar
                    has_sunday = True
                    sunday_weeks.add(week_start)
                elif scheduled_weekday == 0:  # Pazartesi
                    has_monday = True
                    monday_weeks.add(week_start)
        
        target_week_start = target_date - timedelta(days=target_weekday)
        
        if target_weekday == 6 and has_monday:  # Pazar atanıyor
            if target_week_start in monday_weeks:
                return True  # Aynı hafta, yasak
        
        elif target_weekday == 0 and has_sunday:  # Pazartesi atanıyor
            if target_week_start in sunday_weeks:
                return True  # Aynı hafta, yasak
    ...
    def has_consecutive_days_conflict(self, person_id, target_date, existing_schedule):
        """
        Ardışık gün çakışma kontrolü - USER PROVEN + SUNDAY-MONDAY EXCEPTION
        Aynı kişiye peş peşe günlerde nöbet verilemez
        EXCEPTION: Zorunlu eşleştirmeler için Pazar→Pazartesi farklı haftalarda izin verilir
        Args: person_id, target_date, existing_schedule
        ...
        for scheduled_date, scheduled_person, _ in existing_schedule:
            if scheduled_person == person_id and (scheduled_date == prev_day or scheduled_date == next_day):
                # EXCEPTION: Pazar→Pazartesi zorunlu eşleştirmesi için farklı hafta kontrolü
                if (scheduled_date.weekday() == 6 and target_date.weekday() == 0):  # Pazar→Pazartesi
                    scheduled_week = scheduled_date.isocalendar()[1]
                    target_week = target_date.isocalendar()[1]
                    if scheduled_week != target_week:
                        continue  # Farklı hafta, izin ver
                
                return True  # Diğer tüm ardışık günler yasak
    ...
    def generate_schedule(self, year, month):
        """
        Ana nöbet programı oluşturma algoritması - USER PROVEN + NEW REQUIREMENTS
